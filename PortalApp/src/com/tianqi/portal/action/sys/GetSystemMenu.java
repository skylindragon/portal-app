...
public class GetSystemMenu extends ActionBase {
	...
	public String execute(HttpServletRequest req, HttpServletResponse rsp) throws Exception {
		...
		try {
			...
			pRspObj.put("RET_MSG", "菜单查询成功");
		} catch (Exception e) {
			e.printStackTrace();
			pRspObj.put("RET_CODE", "-1");
			pRspObj.put("RET_MSG", "菜单查询失败" + ((e.getMessage() == null)?e.getCause().getMessage():e.getMessage()));
		}
