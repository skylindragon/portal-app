	...
	$.fn.navigation = function(options){
		...
		return this.each(function() {
			...
			this.binddragable = function(){
				...
				$.each(dragdiv,function(k,drag){
					...
					drag.mouseup(function() {  
						...
						var order = obj.n;
						if(order===0){
							drag.stop().animate({left:startx+"px",top:starty+"px"},300);
						}else{
							if(order>opts.rows.length-1&&opts.rows.length>2){
			...
			this.getOrder = function(para){
				...
				var _x = para.x+dragdivwidth;
				var _y = para.y+dragdivheight;
				var rx = 0;
				var cy = 0;
				var left = 0;
				var top = 0;
				for(var i=0;i<xArray.length;i++){
					if(i!==xArray.length-1&&_x>xArray[i]+dragdivwidth/2&&_x<xArray[i+1]+dragdivwidth/2){
						rx = i;
						left = xArray[i];
						break;
					}
					if(i===xArray.length-1&&_x>xArray[i]+dragdivwidth/2){
						rx = i;
						left = xArray[i];
						break;
					}
				...
				for(var i=0;i<yArray.length;i++){
					if(i!==yArray.length-1&&_y>yArray[i]+dragdivheight/2&&_y<yArray[i+1]+dragdivheight/2){
						cy = i;
						top = yArray[i];
						break;
					}
					if(i===yArray.length-1&&_y>yArray[i]+dragdivheight/2){
						cy = i;
						top = yArray[i];
						break;
					}
